...
void RemoteControlProcess(Remote *rc)
{
    ...
    {
        ChassisSpeedRef.forward_back_ref = 0.9*(rc->ch1 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT;
        ChassisSpeedRef.left_right_ref   = 1.4*(rc->ch0 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT; 
    }
    ...
    if(GetWorkState() == NORMAL_STATE)
    {
		...
		//printf("pitch_angle_dynamic_ref:%f\tyaw_angle_dynamic_ref:%f\n",GimbalRef.pitch_angle_dynamic_ref,GimbalRef.yaw_angle_dynamic_ref);
	...
	//printf("pitch_pitch_speed_ref:%f\tyaw_speed_ref:%f\n",GimbalRef.pitch_speed_ref,GimbalRef.yaw_speed_ref);
	GimbalAngleLimit();
	//横向拉杆运动，向上闭合
	if((rc->ch3)>1236)
	{
